 ...
 *
 * CLI:
 *   node --experimental-strip-types cli.mjs --reports-dir <path> \
 *     [--layer 1|2|3|4|5|6|all,comma-list] [--pages slug,...] \
 *     [--strict] [--no-screenshots] [--explain]
 *
 * Requires Node 22.6+ (--experimental-strip-types).
 * Uses dynamic import() so --explain exits before check modules are loaded.
 */
...
/** @type {import('./types.ts').CheckContext} */
const ctx = {
  reportsDir,
  pagesDir,
  sourcesDir,
...
if (argv['pages']) {
  ctx.pagesFilter = argv['pages'].split(',').map(s => s.trim()).filter(Boolean);
}
...
async function runLayer(num, label, fn) {
  const t0 = Date.now();
  let findings = [];
  try {
    findings = await fn();
...
if (layersToRun.has(3) || layersToRun.has(4)) {
  ...
  try {
    ...
    pages = entries
      .filter(e => e.endsWith('.md') && !e.endsWith('+layout.md'))
      .map(e => '/' + e.replace(/\.md$/, '').replace(/index$/, '').replace(/\\/g, '/'))
      .map(r => (r.endsWith('/') && r !== '/') ? r.slice(0, -1) : r);
    if (ctx.pagesFilter && ctx.pagesFilter.length > 0) {
      pages = pages.filter(p => ctx.pagesFilter.some(pf => p.includes(pf)));
    }
  } catch {
