...
from insight_kit.libs.validation import check_claim_id_unique
...
def cmd_preflight(args: argparse.Namespace) -> int:
    ...
    cli_ts = Path(__file__).resolve().parents[4] / "viz" / "core" / "cli.ts"
...
def cmd_viz_install(args: argparse.Namespace) -> int:
    ...
    install_ts = (
        Path(__file__).resolve().parents[4] / "viz" / "evidence" / "install-cli.ts"
    )
