...
def test_claim_status_default():
    c = Claim(claim_id="TST-D-002", statement="s")
    assert c.status == "proposed"


# U-06
def test_claim_confidence_default():
    c = Claim(claim_id="TST-D-003", statement="s")
    assert c.confidence == "medium"


# U-07
def test_claim_to_json_drops_none_top_level():
    c = Claim(claim_id="TST-D-004", statement="s")
    d = c.to_json()
...
def test_claim_to_json_keeps_empty_input_claims():
    c = Claim(claim_id="TST-D-005", statement="s")
    d = c.to_json()
...
def test_claim_to_json_keeps_empty_supports():
    c = Claim(claim_id="TST-D-006", statement="s")
    d = c.to_json()
    assert d["supports"] == []
...
def test_claim_tier_is_dataclass_unconstrained():
    ...
    c = Claim(claim_id="TST-D-007", statement="s", tier="bogus_tier")  # type: ignore[arg-type]
